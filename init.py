...
class Bootstrap:
    def __init__(self, *, update_existing: bool = False) -> None:
        ...
        self._pwndbg_launcher_probe_cache: subprocess.CompletedProcess[str] | None = None
    ...
    def install_pwndbg_uv(self, *, force: bool = False) -> bool:
        ...
        self._pwndbg_launcher_probe_cache = None
    ...
    def remove_legacy_pwndbg_source_lines(self, path: Path) -> bool:
        """Remove obsolete hand-written Pwndbg gdbinit source directives."""
        try:
            existing = path.read_text(encoding="utf-8")
        except FileNotFoundError:
            return False
    ...
    def remove_legacy_pwndbg_portable(self) -> None:
        ...
        changed |= self.remove_legacy_pwndbg_source_lines(GDBINIT)
...
def _init_resolve_address(expression):
...
def _init_proc_mappings():
    ...
    mappings = []
    try:
        with open(f"/proc/{{pid}}/maps", encoding="utf-8", errors="replace") as stream:
            for line in stream:
                fields = line.rstrip("\\n").split(None, 5)
                if len(fields) < 5:
                    continue
                ...
                mappings.append(
                    (int(start_text, 16), int(end_text, 16), int(fields[2], 16), path)
                )
...
def _init_target_for_address(address):
    mappings = _init_proc_mappings()
    selected = next(
        (
            mapping
            for mapping in mappings
            if mapping[0] <= address < mapping[1]
            and mapping[3]
            and not mapping[3].startswith("[")
        ),
        None,
    )
    if selected is not None:
        target = _init_real_path(selected[3])
        matching = [
            mapping
            for mapping in mappings
            if _init_real_path(mapping[3]) == target
        ]
        base = min(start - offset for start, _end, offset, _path in matching)
    ...
    try:
        ...
        filename = None
    if not filename:
        raise RuntimeError("no executable is loaded")
    target = _init_real_path(filename)
    if not Path(target).is_file():
        raise RuntimeError(f"executable not found: {{target}}")
    matching = [
        mapping
        for mapping in mappings
        if _init_real_path(mapping[3]) == target
    ...
    base = min(
        (start - offset for start, _end, offset, _path in matching),
        default=None,
    )
    return target, base


def _init_is_dynamic_elf(filename):
    try:
...
def _init_decompile_with_pwndbg(address):
    if _INIT_R2PIPE is None:
        return None
    try:
        try:
            pid = int(gdb.selected_inferior().pid)
        ...
        try:
            ...
            filename = ""
        target_key = (filename, pid)
        if target_key not in _INIT_ANALYZED_TARGETS:
            analysis = gdb.execute("r2pipe aaa", to_string=True)
            lowered = analysis.lower()
            if "could not import r2pipe" in lowered or "must be installed" in lowered:
                return None
            _INIT_ANALYZED_TARGETS.add(target_key)
        output = gdb.execute(f"r2pipe pdg @ {{address:#x}}", to_string=True)
        lowered = output.lower()
        if not output.strip() or any(
            marker in lowered
            for marker in (
                "could not import r2pipe",
...
def _init_decompile_with_external_r2(address):
    r2 = shutil.which("r2")
    if r2 is None:
        raise RuntimeError("radare2 command not found in PATH")
    filename, base = _init_target_for_address(address)
    command = [r2, "-q", "-e", "scr.color=0", "-e", "io.cache=true"]
    if base is not None and _init_is_dynamic_elf(filename):
        command.extend(["-B", hex(base)])
    command.extend([
        "-c",
        f"aaa; s {{address:#x}}; af; pdg",
        filename,
    ])
    try:
        result = subprocess.run(
        ...
        raise RuntimeError("r2ghidra timed out after 180 seconds") from exc
    output = (result.stdout or "").strip()
    lowered = output.lower()
    if result.returncode != 0 or not output or any(
        marker in lowered
        for marker in (
            "cannot find function",
            "unknown command 'pdg'",
        ...
        details = (result.stderr or output or "unknown r2ghidra error").strip()
...
class InitGhidraCommand(gdb.Command):
    ...
    def invoke(self, argument, from_tty):
        ...
        try:
            address = _init_resolve_address(expression)
            ...
            return
        try:
            output = _init_decompile_with_pwndbg(address)
            backend = "pwndbg-r2pipe"
            if output is None:
                output = _init_decompile_with_external_r2(address)
                backend = "external-r2"
            print(output)
            if os.environ.get("INIT_GHIDRA_PROBE") == "1":
                print(f"INIT_GHIDRA_OK={{backend}}")
        except Exception as exc:
            print(f"ghidra: decompilation failed: {{exc}}")
...
'''
    ...
    def configure_pwndbg_system_gdb(self) -> bool:
        ...
        try:
            ...
            self.remove_legacy_pwndbg_source_lines(GDBINIT)
    ...
    def pwndbg_r2ghidra_probe(self) -> subprocess.CompletedProcess[str]:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="init-ghidra-probe-") as directory:
                ...
                build = self.run(
                    ["gcc", "-g", "-O0", "-fno-inline", "-no-pie", str(source), "-o", str(binary)],
                    check=False,
                    capture=True,
                    timeout=30,
                )
                if build.returncode != 0:
                    self._pwndbg_probe_cache = build
                    return self._pwndbg_probe_cache
                self._pwndbg_probe_cache = self.run(
                    ...
                    capture=True,
                    timeout=240,
                    env={"INIT_GHIDRA_PROBE": "1"},
                )
        except OSError as exc:
            self._pwndbg_probe_cache = subprocess.CompletedProcess(
                command, 1, "", f"Ghidra probe setup failed: {exc}"
            )
        return self._pwndbg_probe_cache
    ...
    def pwndbg_launcher_probe(self) -> subprocess.CompletedProcess[str]:
        ...
        tool_root = self.pwndbg_tool_root()
        launcher = tool_root / "bin" / "pwndbg" if tool_root is not None else None
        if launcher is None or not launcher.is_file() or not os.access(launcher, os.X_OK):
            self._pwndbg_launcher_probe_cache = subprocess.CompletedProcess(
                ["pwndbg"], 1, "", "uv-managed Pwndbg launcher unavailable"
            )
            return self._pwndbg_launcher_probe_cache
        self._pwndbg_launcher_probe_cache = self.run(
            [
                str(launcher), "--batch",
                "-ex", "set debuginfod enabled off",
                "-ex", "pi import pwndbg; print('INIT_PWNDBG_LAUNCHER_OK')",
            ],
            check=False,
            capture=True,
            timeout=90,
    ...
    def pwndbg_launcher_available(self) -> bool:
        result = self.pwndbg_launcher_probe()
        output = ((result.stdout or "") + (result.stderr or "")).lower()
        return (
            result.returncode == 0
            and "init_pwndbg_launcher_ok" in output
            and "double-loading of pwndbg" not in output
            and "traceback" not in output
        )
    ...
    def probe_error_details(result: subprocess.CompletedProcess[str]) -> str:
        ...
        useful = [
            ...
            if any(marker in line.lower() for marker in (
                ...
                "cannot find", "no such file", "double-loading",
            ))
        ]
    ...
    def install_pwndbg_environment(self) -> None:
        ...
        if not self.pwndbg_imports_available(import_probe):
            ...
            return
        self._pwndbg_launcher_probe_cache = None
        launcher_probe = self.pwndbg_launcher_probe()
        if not self.pwndbg_launcher_available():
            details = self.probe_error_details(launcher_probe)
            suffix = f": {details}" if details else ""
            self.failures.append("Pwndbg launcher failed or loaded twice" + suffix)
            return
        ...
        if (
            ...
            self.ok(
                "system GDB and the pwndbg launcher each load Pwndbg once; "
